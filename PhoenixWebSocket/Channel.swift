...
open class Channel {
    ...
    open internal(set) var status: Status {
    ...
    open var joinPayload: Message.JSON?
    ...
    open func onStatusChange(_ callback: @escaping (Status) -> ()) -> Self {
        onStatus = callback
        return self
    }
    
    open func on(_ event: String, callback: @escaping (Message) -> ()) -> Self {
